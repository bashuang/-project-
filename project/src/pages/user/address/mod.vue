<template>
    <div class="page">
        ...
        <div class='main'>
            ...
            <button type="button" class='submit-save' @click="submitAdd()">修改</button>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            ...mapActions({
                ...
                anyceDelAddress:"userAddress/delAddress",
            }),
            // 点击添加地址事件
            submitAdd(){
                ...
                this.modAddress({aid:this.aid,uid:this.uid,name:this.name,cellphone:this.cellphone,province:this.province,city:this.city,area:this.area,isdefault:this.isdefault?'1':'0',address:this.address,success:(res)=>{
                        if(res.code==200){
                            this.$router.go(-1)
                        }else{
                            this.$createToast({
            ...
            delAddress(){
                this.$createDialog({
                    ...
                    onConfirm: () => {
                        this.anyceDelAddress({uid:this.uid,aid:this.aid,success:(res)=>{
                            if(res.code==200){
                                this.$router.go(-1)
                            }
                       }})
